(() => {
  ...
  const state = {
    platform: 'all', product: 'all', status: 'all', search: '', sort: 'new', selected: new Set(), tagFilter: new Set(), importTags: [], importPlatforms: ['douyin', 'kuaishou', 'channels', 'xiaohongshu', 'other'], importPlatformNames: {douyin:'抖音',kuaishou:'快手',channels:'视频号',xiaohongshu:'小红书',other:'其他'},
  ...
  function renderProductMenu(){const menu=$('#rvProductMenu');if(!menu)return;menu.innerHTML=`<button data-product="all">全部产品</button><button data-product="none">未关联产品</button>${state.products.map(item=>`<button data-product="${item.id}">${item.name}</button>`).join('')}`}
  function filtered(){
    const lower=state.search.trim().toLowerCase();
    const out=state.videos.filter(v => (state.platform==='all'||v.platform===state.platform) && (state.product==='all'||(state.product==='none'?!v.productId:v.productId===state.product)) && (state.status==='all'||v.state===state.status) && (!lower || `${v.title} ${productName(v.productId)} ${(v.tags||[]).join(' ')}`.toLowerCase().includes(lower)) && [...state.tagFilter].every(t=>v.tags.includes(t)));
  ...
  function card(v){ const selected=state.selected.has(v.id),relation=productName(v.productId),sourceClass=v.source==='本地'?'local':'collect';return `<article class="rv-card ${selected?'selected':''}" data-id="${v.id}"><div class="rv-cover" data-preview><span class="rv-state ${v.state}"><i></i>${states[v.state]}</span><button class="rv-select" data-select aria-label="选择视频" aria-pressed="${selected}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M5 12l5 5L20 7"/></svg></button><span class="rv-play">▶</span><span class="rv-cover-source ${sourceClass}">${v.source}</span><span class="rv-duration">00:${String(v.duration).padStart(2,'0')}</span></div><div class="rv-info" data-detail><div class="rv-title" title="${v.title}">${v.title}</div><div class="rv-product-platform-row"><span class="rv-card-tag product ${relation?'':'empty'}" title="${relation||'未关联产品'}">${relation||'未关联产品'}</span><span class="rv-card-tag platform">${names[v.platform]}</span></div><div class="rv-meta">${v.uploadedAt}</div><button class="rv-card-more" data-more aria-label="更多操作">⋯</button></div><div class="rv-card-menu" hidden><button data-action="download">下载</button><button data-action="analyze" ${v.state==='done'?'disabled aria-disabled="true" title="视频已分析，无需重复分析"':''}>分析</button><i class="rv-menu-divider"></i><button data-action="rename">重命名</button><button data-action="tag">编辑标签</button><button data-action="product">${relation?'更换关联产品':'关联产品'}</button><i class="rv-menu-divider"></i><button data-action="delete" class="danger">删除</button></div></article>`}
  function render(){ const list=filtered();$('#rvGrid').innerHTML=list.map(card).join('');$('#rvEmpty').hidden=!!list.length;renderSelection();renderProductMenu();$('#rvPlatformText').textContent=state.platform==='all'?'全部平台':names[state.platform];$('#rvProductText').textContent=state.product==='all'?'全部产品':state.product==='none'?'未关联产品':productName(state.product)||'全部产品';$('#rvStatusText').textContent=state.status==='all'?'全部状态':states[state.status];$('#rvSearchClear').hidden=!state.search }
  ...
  function preview(v){
    const node=showOverlay(`<header class="rv-modal-head"><div><small>参考视频预览</small><h2 data-preview-title>${v.title}</h2></div><button class="rv-close" data-close>×</button></header><div class="rv-modal-body rv-preview-body"><div class="rv-preview-video"><span class="rv-play">▶</span></div><div class="rv-info-panel"><div class="rv-info-panel-head"><div class="rv-pills"><span class="rv-pill source" data-preview-source>${v.source}</span><span class="rv-pill ${v.state}" data-preview-status>${states[v.state]}</span><span class="rv-pill" data-preview-platform>${platformLabel(v.platform)}</span></div><button class="rv-edit-pill" data-edit-info type="button">编辑</button></div><div class="rv-kv"><div><span>视频名称</span><b data-preview-name>${v.title}</b></div><div><span>关联产品</span><b class="rv-relation-value" data-preview-product>${productName(v.productId)||'未关联产品'}</b></div><div><span>时长 / 大小</span><b>00:${String(v.duration).padStart(2,'0')} / ${v.size}</b></div><div><span>上传时间</span><b>${v.uploadedAt}</b></div></div><section class="rv-tag-section"><div><span>视频标签</span><button class="rv-link-btn" data-edit-tags>＋ 添加标签</button></div><div class="rv-tags" data-preview-tags>${previewTags(v)}</div></section></div></div><footer class="rv-modal-foot">${previewFooter(v)}</footer>`);bindModal(v,node);
  }
  ...
  function detail(v){
    ...
    page.innerHTML = `<header class="rv-detail-page-head">
      <button class="rv-back" data-back>← 返回列表</button>
      <div class="rv-detail-page-title"><div><b>${v.title}</b><span class="rv-pill done">已分析</span></div><p>关联产品：<strong class="rv-detail-product">${productName(v.productId)||'未关联产品'}</strong><i>·</i>来源平台：${names[v.platform] || v.platform}<i>·</i>上传方式：${v.source}<i>·</i>上传时间：${v.uploadedAt}<i>·</i>标签：${tags || '<span class="rv-detail-meta-empty">暂无标签</span>'}</p></div>
      <div class="rv-detail-page-actions"><button class="rv-btn rv-btn-secondary" data-edit-detail>编辑信息</button><button class="rv-btn rv-btn-primary" data-analyze="${v.id}">重新分析</button></div>
    </header>
  ...
  let searchTimer;
  $('#rvImport').onclick=()=>importModal();$('#rvCollect').onclick=()=>toast('采集插件能力建设中，后续将在此跳转插件下载页。');$('#rvPlatformBtn').onclick=()=>openMenu($('#rvPlatformBtn'),$('#rvPlatformMenu'));$('#rvPlatformMenu').onclick=e=>{const b=e.target.closest('[data-platform]');if(b){state.platform=b.dataset.platform;closeMenus();render()}};$('#rvProductBtn').onclick=()=>openMenu($('#rvProductBtn'),$('#rvProductMenu'));$('#rvProductMenu').onclick=e=>{const b=e.target.closest('[data-product]');if(b){state.product=b.dataset.product;closeMenus();render()}};$('#rvStatusBtn').onclick=()=>openMenu($('#rvStatusBtn'),$('#rvStatusMenu'));$('#rvStatusMenu').onclick=e=>{const b=e.target.closest('[data-status]');if(b){state.status=b.dataset.status;closeMenus();render()}};$('#rvSort').onclick=()=>openMenu($('#rvSort'),$('#rvSortMenu'));$('#rvSortMenu').onclick=e=>{const b=e.target.closest('[data-sort]');if(b){state.sort=b.dataset.sort;closeMenus();render()}};$('#rvSearch').oninput=e=>{clearTimeout(searchTimer);state.search=e.target.value;$('#rvSearchClear').hidden=!state.search;searchTimer=setTimeout(render,300)};$('#rvSearchClear').onclick=()=>{clearTimeout(searchTimer);state.search='';$('#rvSearch').value='';render();$('#rvSearch').focus()};$('#rvRefresh').onclick=()=>{render();toast('列表已刷新')};$('#rvReset').onclick=()=>{state.platform='all';state.product='all';state.status='all';state.search='';state.tagFilter.clear();$('#rvSearch').value='';render()};$('#rvTagBtn').onclick=()=>tagModal([],null,'filter');
  ...
  window.addEventListener('message',event=>{if(event.source!==window.parent||event.data?.type!=='rv-products'||!Array.isArray(event.data.products))return;state.products=event.data.products.filter(item=>item?.id&&item?.name);if(state.product!=='all'&&state.product!=='none'&&!state.products.some(item=>item.id===state.product))state.product='all';render()});
  ...
  window.addEventListener('message',event=>{
    ...
    state.platform='all';state.product='all';state.status='all';state.search='';state.tagFilter.clear();$('#rvSearch').value='';
