(() => {
  ...
  const states = {pending:'待拆解',running:'拆解中',done:'已拆解',failed:'拆解失败'};
  ...
  function previewFooter(v){return `${v.state==='done'?'<button class="rv-btn rv-btn-secondary" data-view-result>查看拆解结果</button>':`<button class="rv-btn rv-btn-secondary" data-analyze="${v.id}" ${v.state==='running'?'disabled':''}>${v.state==='running'?'拆解中':v.state==='failed'?'重新拆解':'开始拆解'}</button>`}<button class="rv-btn rv-btn-primary" data-download="${v.id}">下载视频</button>`}
  ...
  function detail(v){
    ...
    const audit=auditMeta(v); page.innerHTML = `<header class="rv-detail-page-head">
      ...
      <div class="rv-detail-page-title"><div><b>${v.title}</b><span class="rv-pill done">已拆解</span></div><p>关联产品：<strong class="rv-detail-product">${productName(v.productId)||'未关联产品'}</strong><i>·</i>来源平台：${names[v.platform] || v.platform}<i>·</i>上传方式：${v.source}<i>·</i>标签：${tags || '<span class="rv-detail-meta-empty">暂无标签</span>'}</p><p>上传：${audit.uploader} · ${auditTime(v.uploadedAt,true)}<i>·</i>最近修改：${audit.updatedBy} · ${auditTime(audit.updatedAt,true)}</p></div>
      <div class="rv-detail-page-actions"><button class="rv-btn rv-btn-secondary" data-history>查看变更</button><button class="rv-btn rv-btn-secondary" data-edit-detail>编辑信息</button><button class="rv-btn rv-btn-primary" data-analyze="${v.id}">重新拆解</button></div>
    </header>
    ...
    $('[data-json]', page).onclick = () => toast(`已下载 ${v.title} V${v.version} 拆解结果 JSON`);
  ...
  function tagModal(videos=[],done,mode='assign',onImportSave=null){
    ...
    const canManageTags=mode!=='filter';
    ...
    const body=()=>`<div class="rv-tag-modal-body"><aside class="rv-tag-modal-side">${state.tagGroups.map(g=>`<button type="button" class="rv-tag-group ${g.id===draft.group?'active':''}" data-tag-group="${g.id}"><span>${g.name}</span><b>${g.id==='all'?state.tags.length:state.tags.filter(t=>state.tagGroupMap[t]===g.id).length}</b></button>`).join('')}${canManageTags?'<i></i><button type="button" class="rv-new-tag-group" data-new-group>＋ 新建分组</button>':''}</aside><div class="rv-tag-modal-main"><label class="rv-tag-search"><span>⌕</span><input id="rvTagSearch" placeholder="搜索标签..."></label><div id="rvTagChoices">${tagPicker(selected,draft.group,draft.keyword)}</div>${canManageTags?'<div class="rv-tag-create"><button type="button" class="rv-new-tag" data-new-tag>＋ 新建标签</button></div>':''}<small id="rvTagError"></small></div></div>`;
  ...
  function confirmAnalyze(videos,onDone){const valid=videos.filter(v=>v.state!=='running');if(!valid.length)return toast('没有可拆解的视频');const node=showOverlay(`<header class="rv-modal-head"><div><small>视频拆解</small><h2>确认开始${valid.length>1?'批量':''}拆解？</h2></div><button class="rv-close" data-close>×</button></header><div class="rv-modal-body"><ul class="rv-confirm-list"><li>生成口播文案、脚本结构、画面总结与分镜结果</li></ul><p class="rv-modal-note">拆解期间可继续浏览，完成后可在视频详情查看结果。</p>${videos.some(v=>v.state==='running')?'<p class="rv-note">拆解中的视频已跳过。</p>':''}</div><footer class="rv-modal-foot"><button class="rv-btn rv-btn-secondary" data-close>取消</button><button class="rv-btn rv-btn-primary" data-confirm>开始拆解</button></footer>`);const root=node;$$('[data-close]',root).forEach(b=>b.onclick=()=>closeOverlay(root));$('[data-confirm]',root).onclick=e=>{e.currentTarget.disabled=true;e.currentTarget.textContent='拆解中…';valid.forEach(v=>v.state='running');render();setTimeout(()=>{valid.forEach(v=>{v.state='done';v.version=(v.version||0)+1});closeOverlay(root);render();onDone?.();toast(`已完成 ${valid.length} 个视频的拆解`)},800)}}
