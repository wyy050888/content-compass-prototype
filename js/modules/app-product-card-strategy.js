(function () {
  ...
  function normalizeRule(rule, index) {
    ...
    return { id: rule.id || nextId("RULE"), images, prompt: rule.prompt || "", promptState: rule.promptState || (rule.prompt ? "ready" : "idle"), promptVersion: 0, quantity: Number(rule.quantity || 2), isNew: false };
  }
  ...
  function sourcePicker() {
    ...
    return `<div class="pc-source-picker"><div class="pc-source-preview">${image ? imageVisual(image) : "1:1<br>商品主图"}</div><div class="pc-source-actions"><strong>${image ? app.escape(image.name) : "选择一张商品主图"}</strong><small>仅支持比例为 1:1 的图片${app.tip("本地上传会读取图片真实宽高；宽高不相等时不会加入任务。")}</small><button type="button" data-source-local>本地上传</button><button type="button" data-source-library>从图片库选择</button></div></div>`;
  }
  function strategyBody(task) {
    return `<div class="pc-form"><div class="pc-form-grid"><label class="pc-field"><span>任务名称</span><input id="pcTaskName" maxlength="40" value="${app.escape(task?.name || "")}" ${editor.editable ? "" : "disabled"}></label><label class="pc-field"><span>产品名称</span><select id="pcTaskProduct" ${editor.editable && editor.isNew ? "" : "disabled"}>${app.data.products.map(product => `<option value="${product.id}" ${product.id === editor.productId ? "selected" : ""}>${app.escape(product.name)}</option>`).join("")}</select></label>${editor.isNew ? `<div class="pc-field pc-field-wide"><span>商品主图</span>${sourcePicker()}</div>` : ""}</div>${task?.failureReason ? `<div class="pc-note-error">${app.escape(task.failureReason)}</div>` : ""}<div id="pcUploadFeedback" aria-live="polite"></div><input type="file" id="pcSourceLocalInput" accept="image/*" hidden><input type="file" id="pcRuleLocalInput" accept="image/*" multiple hidden><div id="pcRuleEditor"></div></div>`;
  }
  ...
  function generatedPrompt(rule, index) {
    const imageNames = rule.images.map(image => image.name.replace(/\.[^.]+$/, "")).join("、") || "垫图";
    const base = app.data.prompts[index % app.data.prompts.length];
    return `${base}；综合参考“${imageNames}”的主体、构图和色彩，形成差异化方案 ${index + 1}`;
  }
    ...
    for (let offset = 0; offset < jobs.length; offset += 5) {
      ...
      window.setTimeout(() => {
        ...
        batch.forEach(job => {
          ...
          rule.prompt = generatedPrompt(rule, index);
          rule.promptState = "ready";
  ...
  function openStrategy(task, isNew, preferredProductId = "") {
    ...
    app.openDrawer({ mode: editor.editable ? "edit" : "view", eyebrow: isNew ? "图片生成" : task.id, title: isNew ? "新建生图任务" : editor.editable ? "编辑生成策略" : "查看生成策略", subtitle: editor.editable ? "商品主图生成策略" : `${editor.rules.length} 条规则，共 ${task.target} 张`, body: strategyBody(task), footer: strategyFooter() });
    renderRules();
  ...
  function readFile(file) {
    return new Promise(resolve => {
      if (!file?.type?.startsWith("image/")) { resolve({ valid: false, name: file?.name || "未知文件" }); return; }
      const reader = new FileReader();
      reader.onerror = () => resolve({ valid: false, name: file.name });
      reader.onload = () => {
        const image = new Image();
        image.onerror = () => resolve({ valid: false, name: file.name });
        image.onload = () => resolve(image.naturalWidth === image.naturalHeight ? { valid: true, image: { id: nextId("LOCAL"), name: file.name, url: String(reader.result), width: image.naturalWidth, height: image.naturalHeight, tone: uid % 6 + 1, source: "local" } } : { valid: false, name: file.name, width: image.naturalWidth, height: image.naturalHeight });
  ...
  async function validateFiles(files) {
    ...
    if (invalid.length) {
      const details = invalid.slice(0, 3).map(item => `${item.name}${item.width && item.height ? `（${item.width}×${item.height}）` : ""}`).join("、");
      const more = invalid.length > 3 ? `、等 ${invalid.length} 张` : "";
      const message = `${details}${more}比例不符合 1:1，上传失败${valid.length ? `；其余 ${valid.length} 张已正常加入` : ""}`;
      if (feedback) feedback.innerHTML = `<div class="pc-upload-error"><span><b>图片比例不符合要求</b>${app.escape(message)}</span><button type="button" data-upload-error-close aria-label="关闭提示">×</button></div>`;
      app.toast(`${invalid.length} 张图片比例不符合 1:1，上传失败`);
  ...
  function setFileTarget(target) {
    ...
    input.multiple = target.mode !== "replace" && target.mode !== "batch";
    input.value = "";
    input.click();
  }
  ...
  function validateSubmit() {
    ...
    if (!editor.rules.length) return "请至少创建 1 条规则";
    if (editor.rules.length > 50) return "一个任务最多 50 条规则";
    const generating = generatingCount();
    if (generating) return `还有 ${generating} 条提示词正在生成`;
    if (editor.rules.some(rule => !rule.images.length)) return "每条规则至少需要一张 1:1 垫图";
    if (editor.rules.some(rule => !rule.prompt.trim())) return "每条规则都需要提示词";
    if (editor.rules.some(rule => rule.quantity < 1 || rule.quantity > 20)) return "单条规则生图数量需为 1–20 张";
    const total = totalQuantity();
    if (total > 200) return `总生图数量超出 ${total - 200} 张`;
    return "";
  }
  function persistDraft(status) {
    ...
    const task = existing || { id: `GT-0902-${String(nextNumber).padStart(3, "0")}`, creator: "周宁", success: 0, failed: 0, createdAt };
    Object.assign(task, { name: values.name || "未命名生图任务", productId: values.productId, sourceTaskId: editor.sourceTaskId, rules: editor.rules.map(rule => ({ id: rule.id, images: rule.images.map(copyImage), prompt: rule.prompt, promptState: rule.promptState, quantity: rule.quantity })), target: totalQuantity(), status });
  ...
  app.els.drawerBody.addEventListener("click", async event => {
    ...
    if (imageAction && rule) {
      ...
      if (imageAction === "delete" && imageNode) { rule.images = rule.images.filter(image => image.id !== imageNode.dataset.imageId); renderRules(); if (rule.images.length) schedulePromptGeneration([rule.id]); app.markDrawerDirty(); }
      return;
    }
    ...
    if (event.target.closest("#pcAddRule")) {
      if (editor.rules.length >= 50) { app.toast("一个任务最多 50 条规则"); return; }
    ...
    if (action && rule) {
      const index = editor.rules.indexOf(rule);
      if (action === "insert") {
        const count = Math.max(1, Number(row.querySelector(".pc-inline-input")?.value || 1));
        const remaining = 50 - editor.rules.length;
        if (count > remaining) { app.toast(remaining ? `当前已有 ${editor.rules.length} 条规则，最多还能插入 ${remaining} 条` : "当前已有 50 条规则，不能继续插入"); return; }
      ...
      if (action === "copy") {
        if (editor.rules.length >= 50) { app.toast("当前已有 50 条规则，不能继续复制"); return; }
  ...
  app.els.drawerBody.addEventListener("input", event => {
    ...
    if (event.target.matches("[data-rule-prompt]") && rule) rule.prompt = event.target.value;
  ...
  app.els.drawerBody.addEventListener("change", async event => {
    ...
    if (event.target.matches("[data-rule-qty]")) { const row = event.target.closest(".pc-rule-row"), rule = editor.rules.find(item => item.id === row?.dataset.ruleId); if (rule) { rule.quantity = Math.max(1, Math.min(20, Number(event.target.value || 1))); event.target.value = rule.quantity; updateSummary(); } }
    if (event.target.matches("#pcSourceLocalInput")) { const images = await validateFiles(event.target.files); if (images[0]) applySourceImage(images[0]); }
    if (event.target.matches("#pcRuleLocalInput")) {
      const images = await validateFiles(event.target.files); if (!images.length || !editor.fileTarget) return;
  ...
  app.els.drawerFoot.addEventListener("click", async event => {
    if (event.target.closest("#pcSaveDraft")) { const task = persistDraft("draft"); app.toast(`草稿 ${task.id} 已保存`); editor.token += 1; app.forceCloseDrawer(); return; }
    if (!event.target.closest("#pcSubmitGeneration") || editor.submitting) return;
    const error = validateSubmit(); if (error) { app.toast(error); return; }
