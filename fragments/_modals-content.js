(function(){var s=document.currentScript;if(!s)return;const p=window.ContentCompassPersonaRules;const createButtons=(values,group)=>values.map(value=>`<button class="cl-audience-chip" type="button" data-value="${value}">${value}</button>`).join("");const editButtons=(values,attr)=>values.map(value=>`<button type="button" data-cl-${attr}="${value}">${value}</button>`).join("");s.insertAdjacentHTML('beforebegin',`
  <!-- 文案库：新增文案 -->
      ...
      <div class="modal-body">
        ...
        <section class="cl-create-section"><div class="cl-create-section-head"><div><h4>产品基础信息</h4><p>关联产品，并补充文案对应的人群信息</p></div></div><div class="cl-create-form">
          ...
          <div class="cl-audience-block">
            ...
            <div class="cl-audience-line"><span>抖音八大人群</span><div class="cl-audience-chips" data-cl-choice-group="audience">${createButtons(p.audiences,"audience")}</div></div>
            <div class="cl-audience-line"><span>性别 <b style="color:#e14b53;">*</b></span><div class="cl-audience-chips" data-cl-choice-group="gender">${createButtons(p.genders,"gender")}</div></div>
            <div class="cl-audience-line"><span>年龄 <b style="color:#e14b53;">*</b></span><div class="cl-audience-chips" data-cl-choice-group="age">${createButtons(p.ages.slice(0,-1),"age")}<button class="cl-audience-chip" type="button" data-value="custom">自定义</button><div class="cl-custom-age" id="clCustomAge"><input type="number" id="clAgeMin" min="1" max="99" placeholder="起始"><i data-cl-age-separator>至</i><input type="number" id="clAgeMax" min="1" max="99" placeholder="结束"><span data-cl-age-open-suffix hidden>岁及以上</span><button type="button" class="age-boundary-toggle" data-cl-age-boundary-toggle>设为以上</button><input type="checkbox" id="clAgePlus" hidden></div></div></div>
          ...
          <div class="cl-edit-form">
            ...
            <div class="cl-edit-field cl-edit-persona-field full" id="clEditPersonaField">
              ...
              <div class="cl-edit-audience-block">
                ...
                <div class="cl-edit-audience-row"><span>抖音八大人群</span><div class="cl-edit-choice-row" data-cl-audience-box>${editButtons(p.audiences,"audience")}</div></div>
                <div class="cl-edit-audience-row"><span>性别</span><div class="cl-edit-choice-row" data-cl-single="gender">${editButtons(p.genders,"gender")}</div></div>
                <div class="cl-edit-audience-row"><span>年龄</span><div class="cl-edit-choice-row" data-cl-single="age">${editButtons(p.ages.slice(0,-1),"age")}<button type="button" data-cl-age="custom">自定义</button><span class="cl-edit-custom-age" data-cl-custom-age hidden><input type="number" id="clEditAgeMin" min="1" max="99" placeholder="起始"><i data-cl-edit-age-separator>至</i><input type="number" id="clEditAgeMax" min="1" max="99" placeholder="结束"><span data-cl-edit-age-open-suffix hidden>岁及以上</span><button type="button" class="age-boundary-toggle" data-cl-edit-age-boundary-toggle>设为以上</button><input type="checkbox" id="clEditAgePlus" hidden></span></div></div>
              </div>
