(function(){var s=document.currentScript;if(!s)return;var p=window.ContentCompassPersonaRules;var audienceButtons=p.audiences.map(function(v){return '<button type="button">'+v+'</button>';}).join('');var genderButtons=p.genders.map(function(v){return '<button type="button">'+v+'</button>';}).join('');var ageButtons=p.ages.map(function(v){return '<button type="button"'+(v==='自定义'?' data-persona-custom-age-trigger':'')+'>'+v+'</button>';}).join('');s.insertAdjacentHTML('beforebegin',`
    </main>
  </div>
    ...
    <div class="modal persona-template-modal">
      ...
      <div class="modal-body">
        <div class="persona-form-grid">
          ...
          <div class="field full"><label>抖音八大人群<em class="required-mark">*</em></label><div class="persona-choice-row" data-persona-form-single="audience">${audienceButtons}</div></div>
          <div class="field full"><label>性别<em class="required-mark">*</em></label><div class="persona-choice-row" data-persona-form-single="gender">${genderButtons}</div></div>
          <div class="field full"><label>年龄<em class="required-mark">*</em></label><div class="persona-choice-row" data-persona-form-single="age">${ageButtons}<span class="persona-custom-age" data-persona-custom-age hidden><input id="personaFormAgeMin" type="number" min="1" max="99" placeholder="起始年龄"><i data-persona-age-separator>至</i><input id="personaFormAgeMax" type="number" min="1" max="99" placeholder="结束年龄"><span data-persona-age-open-suffix hidden>岁及以上</span><button type="button" class="age-boundary-toggle" data-persona-age-boundary-toggle>设为以上</button><input id="personaFormAgePlus" type="checkbox" hidden></span></div></div>
